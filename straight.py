...
class GapFollow(Node):
    def __init__(self):
        ...
        # 각도 안정화 파라미터
        self.prev_best_angle = 0.0         # 이전 프레임의 best_angle 저장
        self.angle_change_threshold = np.radians(5.0)  # 직선 주행 시 최대 각도 변화량 (5도)
    ...
    def find_best_point_stable_angle(self, start_idx, end_idx, ranges, angle_min, angle_increment, 
                                     min_idx, prev_angle, max_angle_change):
        """
        직선 구간에서 이전 각도에 가장 가까운 best point 찾기
        
        Args:
            start_idx, end_idx: gap 범위 (proc 기준 인덱스)
            ranges: 처리된 lidar 배열 (proc)
            angle_min: LiDAR 시작 각도
            angle_increment: 각도 증분
            min_idx: 전역 인덱스 변환용
            prev_angle: 이전 프레임의 각도
            max_angle_change: 허용 최대 각도 변화량 (radians)
        ...
        for local_idx in range(start_idx, end_idx):
            global_idx = local_idx + min_idx
            angle = angle_min + global_idx * angle_increment
            gap_angles.append(angle)
        
        gap_angles = np.array(gap_angles)
        
        # 이전 각도와의 차이 계산
        angle_diffs = np.abs(gap_angles - prev_angle)
        
        # 거리 기반 가중치 (먼 곳 선호)
        distance_weights = ranges[start_idx:end_idx]
        distance_weights = distance_weights / (np.max(distance_weights) + 1e-6)  # 정규화
        
        # 각도 안정성 가중치 (이전 각도에 가까울수록 높음)
        # max_angle_change를 기준으로 지수 감소
        angle_weights = np.exp(-angle_diffs / max_angle_change)
        
        # 종합 점수: 거리 50% + 각도 안정성 50%
        combined_score = 0.5 * distance_weights + 0.5 * angle_weights
        
        # 최대 점수 인덱스 선택
        best_local_in_gap = np.argmax(combined_score)
        best_idx_local = start_idx + best_local_in_gap
    ...
    def lidar_callback(self, data: LaserScan):
        ...
        if is_straight_clear:
            # 직선 구간 → 이전 각도 유지 우선 (각도 변화 최소화)
            if start_f is not None and end_f is not None:
                best_idx_local = self.find_best_point_stable_angle(
                    start_f, end_f, proc, 
                    data.angle_min, data.angle_increment, min_idx,
                    self.prev_best_angle, self.angle_change_threshold
                )
            else:
                best_idx_local = self.find_best_point_stable_angle(
                    start_m, end_m, proc,
